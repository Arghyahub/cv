...
```jsx title="States with useState"
...
`useState` is a React hook that allows you to have state variables in functional components. The first value in the LHS array is the state variable and the second value is the function to update the state.
...
```jsx title="Fetch on load with loader"
...
You will require to learn some external libraries that will help you along your way like `react-router-dom` for routing, UI libraries such as `MUI` or `Shadcn` and some state management libraries like `Redux` or `Zustand`
...
```jsx title="Lazy Loading and Suspense"
...
I'll recommend to make projects, Learn Typescript and explore other libraries out there. Good job, Hope my article helps. If you have any queries, feel free to ask.